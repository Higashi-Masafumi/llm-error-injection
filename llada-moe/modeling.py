    ...
    def forward(
        ...
        **kwargs,
        ...
        if "q_norm" in dir(self):
            query_states = self.q_norm(query_states.reshape(-1, self.head_dim)).reshape(
                bsz, q_len, -1
            )
            key_states = self.k_norm(key_states.reshape(-1, self.head_dim)).reshape(
                bsz, q_len, -1
            )
...
class LLaDAMoESparseMoeBlock(nn.Module):
    def __init__(self, config):
        ...
        self.experts = nn.ModuleList(
            [LLaDAMoEMLP(config, "expert") for _ in range(self.num_experts)]
        )
...
class LLaDAMoEDecoderLayer(nn.Module):
    def __init__(self, config: LLaDAConfig, layer_idx: int):
        ...
        self.self_attn = LLADAMOE_ATTENTION_CLASSES[config._attn_implementation](
            config=config, layer_idx=layer_idx
        )

        self.mlp = (
            LLaDAMoESparseMoeBlock(config)
            if self.mlp_type == "moe"
            else LLaDAMoEMLP(config, "dense")
        )
        self.input_layernorm = LLaDAMoERMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
        self.post_attention_layernorm = LLaDAMoERMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
        if (
            config.shared_expert_intermediate_size is not None
            and self.mlp_type == "moe"
        ):
...
class LLaDAMoEModel(LLaDAMoEPreTrainedModel):
    ...
    def forward(
        ...
        assert (
            not use_cache and past_key_values is None and cache_position is None
...
class LLaDAMoEModelLM(LLaDAMoEPreTrainedModel):
    ...
    def forward(
        ...
        """
        assert (
            labels is None and num_logits_to_keep == 0
        ), "LLaDAMoE model does not support calculate loss in the forward pass."
